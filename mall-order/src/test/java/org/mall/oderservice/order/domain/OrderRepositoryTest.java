...
class OrderRepositoryTest {
    ...
    @Test
    void whenCreateOrderNotAuthenticationThenNoAuditMetadata() {
        var rejectedOrder = OrderService.buildRejectedOrder("1234567890", 3);
        StepVerifier.create(orderRepository.save(rejectedOrder))
    ...
    @Test
    @WithMockUser("rigofunc")
    void whenCreateOrderAuthenticationThenAuditMetadata() {
        var rejectedOrder = OrderService.buildRejectedOrder("1234567890", 3);
        StepVerifier.create(orderRepository.save(rejectedOrder))
                .expectNextMatches(order -> order.createdBy().equals("rigofunc") &&
                        order.lastModifiedBy().equals("rigofunc"))
                .verifyComplete();
    }
}